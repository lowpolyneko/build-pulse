...
impl<T> PatternSet<T> {
    ...
    pub fn grep_matches(&self, field: &str) -> impl Iterator<Item = &Pattern<T>> {
...
impl<T> Pattern<T> {
    ...
    pub fn grep_substr<'a>(&self, field: &'a str) -> impl Iterator<Item = &'a str> {
        self.regex.find_iter(field).map(move |m| m.as_str())
    }
}
...
impl Pattern<InDatabase<TagInfo>> {
    ...
    pub fn grep_issue(&self, field: &ArcStr) -> impl Iterator<Item = Issue> {
        self.grep_substr(&field)
            ...
            .into_iter()
            .map(move |(m, duplicates)| Issue {
                snippet: field.substr_from(m),
                tag_id: self.id,
                duplicates,
            })
    }
